...
CB_Net.XHR = function() { return CB_Net.XHR; };
{
	...
	CB_Net.XHR.call = function(URL, method, data, headers, responseType, mimeType, callbackFunction, callbackFunctionOK, callbackFunctionError, allowedSuccessStatuses, asynchronous, XHR)
	{
		...
		{
			if (typeof(callbackFunction) === "function") { callbackFunction.call(XHR, XHR); }
			else if (typeof(callbackFunctionError) === "function") { callbackFunctionError.call(XHR, XHR, callbackFunctionOK); }
		};
		...
		{
			XHR.onreadystatechange = function() { callbackFunction.call(XHR, XHR); };
		}
			...
			XHR.onreadystatechange = function()
			{
				...
				if (XHR.readyState === 4)
				{
					...
					if (CB_indexOf(allowedSuccessStatuses, parseInt(XHR.status)) !== -1)
					{
						...
						{
							callbackFunctionOK.call(XHR, XHR, callbackFunctionError);
						}
					}
					...
					else
					{
						...
						{
							callbackFunctionError.call(XHR, XHR, callbackFunctionOK);
						}
					}
				}
			}
