...
var CB_Screen = function() { return CB_Screen; };
{
	...
	CB_Screen._noSleep = null; //Keeps the NoSleep object for the NoSleep.js library.
...
//////////////////
	...
	*/
	 ...
	 */
	CB_Screen.unlockOrientation = CB_Screen.unsetOrientation = function()
	{
		var orientationObject = CB_Screen.getOrientationObject();
		if (typeof(orientationObject) !== "undefined" && orientationObject !== null && typeof(orientationObject.unlock) !== "undefined" && orientationObject.unlock !== null)
		{
			return orientationObject.unlock();
		}
		else if (typeof(screen) !== "undefined" && screen !== null)
		{
			...
			{
				return screen.oUnlockOrientation();
			}
			...
			{
				return screen.khtmlUnlockOrientation();
			}
			/*
	 ...
	 */
	CB_Screen.lockOrientation = CB_Screen.setOrientation = function(orientationMode)
	{
		orientationMode = CB_trim(orientationMode).toLowerCase();
		var orientationObject = CB_Screen.getOrientationObject();
		if (typeof(orientationObject) !== "undefined" && orientationObject !== null && typeof(orientationObject.lock) !== "undefined" && orientationObject.lock !== null)
		{
			if (orientationMode === "default") { orientationMode = "natural"; }
			//return orientationObject.lock(orientationMode).catch(function(){}); //Attaching an error-catch to prevent Chrome desktop exception.
			return orientationObject.lock(orientationMode)["catch"](function() {});; //Attaching an error-catch to prevent Chrome desktop exception.
		}
		else if (typeof(screen) !== "undefined" && screen !== null)
		{
			...
			{
				return screen.lockOrientation(orientationMode);
			}
			else if (screen.mozLockOrientation)
			{
				return screen.mozLockOrientation(orientationMode);
			}
			else if (screen.webkitLockOrientation)
			{
				return screen.webkitLockOrientation(orientationMode);
			}
			else if (screen.oLockOrientation)
			{
				return screen.oLockOrientation(orientationMode);
			}
			else if (screen.msLockOrientation)
			{
				return screen.msLockOrientation(orientationMode);
			}
			...
			{
				return screen.khtmlLockOrientation(orientationMode);
			}
		}
		return false;
	}
	 ...
	 *  @function
	 *  @returns {string} Returns a string with the current orientation from the [screen orientation object]{@link https://developer.mozilla.org/en-US/docs/Web/API/Screen/orientation} of the [Screen Orientation API]{@link https://developer.mozilla.org/en-US/docs/Web/API/ScreenOrientation}.
	 */
	CB_Screen.getOrientation = function()
	{
		var orientationObject = CB_Screen.getOrientationObject();
		var orientation = "";
		if (typeof(orientationObject) !== null)
		{
	 ...
	 *  @param {string|boolean} [userScalable='0'] - Desired value for the "user-scalable" parameter.
	 ...
	 *  @returns {string|null} Returns the "content" string generated with the values given after applying to the [Viewport]{@link https://developer.mozilla.org/en-US/docs/Mozilla/Mobile/Viewport_meta_tag} meta tag. If the [Viewport]{@link https://developer.mozilla.org/en-US/docs/Mozilla/Mobile/Viewport_meta_tag} meta tag could not be modified, returns null.
	 */
	CB_Screen.setViewport = function(width, height, userScalable, initialScale, minimumScale, maximumScale, targetDensityDPI)
	{
		...
		userScalable = CB_trim(userScalable);
		...
		if (userScalable && userScalable !== "" && userScalable !== "no" && userScalable !== 0 && userScalable !== "0") { userScalable = 1; } //userScalable = "yes"; }
		else { userScalable = 0; } //userScalable = "no"; }
		if (CB_trim(viewportContent) !== "") { viewportContent += ", "; }
		viewportContent += "user-scalable=" + (viewportContent === 0 ? "no" : "yes");
