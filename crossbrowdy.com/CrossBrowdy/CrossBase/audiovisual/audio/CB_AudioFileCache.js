...
CB_AudioFileCache.prototype.createAudioFile = function(URIs, preferredAPIs, preferredFormats, audioObject, callbackOk, callbackError, storeURIsList, checkAutomatically)
{
	...
	preferredAPIs = CB_AudioDetector.getSupportedAPIs(preferredAPIs, false);
 ...
 *  @param {number} [startPlayingTime=Date.now()] - Used internally to calculate the amount of time (in milliseconds) expired without playing the sound. Internal usage only recommended. 
	...
	if (typeof(freeAudioFile.object) !== "undefined" && freeAudioFile.object !== null)
	{
		...
		(
			function()
			{
				...
				if (typeof(onStop) === "function") { onStop.call(audioObject, soundInstanceId); }
			},
 ...
 */
 ...
 /**
 * Performs a desired action, using the provided function, on all the existing {@link CB_AudioFile} objects or on the desired ones (if provided).
 *  @function
 ...
 *  @param {boolean} [delayBetweenEachAffectsFirst=false] - If set to true, the desired delay (if any) will also affect the first call to the "functionEach" function. Internal usage only recommended.
 ...
 */
CB_AudioFileCache.prototype.executeFunctionAll = function(functionEach, delayBetweenEach, audioFiles, returnSetTimeoutsArray, delayBetweenEachAffectsFirst)
{
	if (typeof(functionEach) !== "function") { return 0; }
	...
	for (var x = 0; x < audioFilesLength; x++)
	{
		...
		if (typeof(audioFiles[x]) !== "undefined" && audioFiles[x] !== null)
		{
			...
			{
				new function(x) //Closure to get unique valor of "x" variable for each loop.
				{
...
/**
 * Tries to play all the {@link CB_AudioFile} objects by calling their {@link CB_AudioFile#play} method internally. If a {@link CB_AudioFile} object cannot be played and it is determined necessary, it will try to reload it internally (by calling the {@link CB_AudioFileCache#removeAudioFile} method).
 *  @function
...
CB_AudioFileCache.prototype.setAudioAPIAll = function(preferredAPIs, callbackOk, callbackError, mandatory, forceReload, audioFiles)
{
		...
		function(error, indexObject, preferredAPIs, URIsList, URIsIndex, currentRetry, APIsTried)
		{
			...
			else
			{
				...
				(
					function()
					{
						setAPIEach.call(thisObject, indexObject, preferredAPIs, URIsList, URIsIndex, currentRetry, APIsTried);
					},
	...
	var setAPIEach =
		function(indexObject, preferredAPIsLocal, URIsList, URIsIndex, currentRetry, APIsTried)
		{
