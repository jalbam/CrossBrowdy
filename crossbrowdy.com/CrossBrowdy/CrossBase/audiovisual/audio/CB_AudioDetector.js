...
var CB_AudioDetector = function() { return CB_AudioDetector; };
{
	 ...
	 *  @param {boolean} [allowEmulation=false] - If set to true, it will also detect as supported emulated audio APIs (as "WAAPI" using [waapisim.js]{@link https://github.com/g200kg/WAAPISim} instead of supported natively).
	 *  @param {boolean} [returnOnEmpty=false] - If set to true and no audio API is supported, it will return the value of this parameter instead of null.
	...
	CB_AudioDetector.getPreferredAPI = function(audioAPIs, allowEmulation, returnOnEmpty)
	{
		...
		//If it is not the first time, returns the same as the first time (from the cache):
		if (typeof(CB_AudioDetector._getPreferredAPIReturnCache[audioAPIs]) !== "undefined")
		{
			if (CB_AudioDetector._getPreferredAPIReturnCache[audioAPIs] === null) { return returnOnEmpty; }
			else { return CB_AudioDetector._getPreferredAPIReturnCache[audioAPIs]; }
		}

		var audioAPIs = CB_AudioDetector.getSupportedAPIs(audioAPIs, allowEmulation);
		...
		//Stores the result in the cache for the next time:
		CB_AudioDetector._getPreferredAPIReturnCache[audioAPIs] = preferredAPI;
	...
	CB_AudioDetector.getSupportedAPIs = function(audioAPIs, allowEmulation)
	{
		...
		{
			return CB_AudioDetector._getSupportedAPIsReturnCache[audioAPIs];
		}
		...
		//Stores the result in the cache for the next time:
		CB_AudioDetector._getSupportedAPIsReturnCache[audioAPIs] = supportedAudioAPIs;
	 ...
	 *  @function
	 *  @param {string} audioAPI - The audio API that we want to check. All existing ones are defined in {@link CB_Configuration.CrossBase.CB_AudioFileCache_PREFERRED_AUDIO_APIS}. For example: "WAAPI" ([HTML5 Web Audio API]{@link https://developer.mozilla.org/en-US/docs/Web/API/Web_Audio_API}), "SM2" ([SoundManager 2]{@link http://schillmania.com/projects/soundmanager2/}), "ACMP" ([Apache Cordova Media Plugin]{@link https://github.com/apache/cordova-plugin-media}) or "AAPI" ([HTML5 Audio API]{@link https://developer.mozilla.org/en-US/docs/Web/HTML/Element/audio}).
	 *  @param {boolean} [allowEmulation=false] - If set to true, it will detect as supported also emulated audio APIs (as "WAAPI" using [waapisim.js]{@link https://github.com/g200kg/WAAPISim} instead of supported natively).
