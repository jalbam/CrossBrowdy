...
CB_Controllers = function() { return CB_Controllers; };
{
	...
	CB_Controllers._gamePadUpdateAll = function()
	{
		...
		for (var j in CB_Controllers._gamepads)
		{
			...
			{
				if (typeof(controller.buttons[i]) !== "object")
				{
	 ...
	 *  @param {boolean} [gamepadIdNumeric=false] - If the "gamepadId" parameter is an empty string, this parameter will be ignored. Otherwise, if the "gamepadId" parameter is not empty and this parameter is set to true, the gamepadId parameter will be converted to integer using "parseInt" internally.
	 ...
	 */
	CB_Controllers.isButtonDown = function(buttonCodes, gamepadId, allPressed, gamepadIdNumeric, caseSensitive)
	{
		//TODO: make it compatible with proprietary gamepads (if possible).
		//TODO: consider to have in mind the axis.
		...
		if (gamepadIdNumeric && gamepadId !== "") { gamepadId = parseInt(gamepadId); }
		...
		for (var x in gamepads)
		{
			...
			(
				...
				|| !gamepadIdNumeric && (caseSensitive && CB_trim(gamepads[x].id) === gamepadId || !caseSensitive && CB_trim(gamepads[x].id).toLowerCase() === gamepadId.toLowerCase())
			)
			{
				for (buttonCode in gamepads[x].buttons)
				{
					if (gamepads[x].buttons[buttonCode].pressed)
					{
	 ...
	 *  @function
	 *  @param {boolean} [avoidProprietary=false] - If set to true, proprietary gamepads which do not use the [GamePad API]{@link https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API} (as WII, WI U gamepads, etc.) will not be included in the returning object and it will only include the ones detected by the [GamePad API]{@link https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API}. Otherwise, if set to true, the returning object will contain two properties, one called "standard" which will contain the gamepads that use the [GamePad API]{@link https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API} and another one called "proprietary" which will contain different properties (one per each proprietary system) and inside of them the gamepad objects for each proprietary system. For gamepads that support the [GamePad API]{@link https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API}, the indexes will be the identifier for each gamepad and the value its gamepad object. For proprietary gamepads, it will contain the returning value of the "getGamePads" function of each proprietary API.
	 ...
	 *  @todo The proprietary ones should return normalize values to simulate the [GamePad API]{@link https://developer.mozilla.org/en-US/docs/Web/API/Gamepad_API}.
	 */
	CB_Controllers.getGamePads = function(avoidProprietary)
	{
		...
		if (!avoidProprietary)
		{
			...
			return {
				standard: CB_Controllers._gamepads,
				proprietary:
				{
