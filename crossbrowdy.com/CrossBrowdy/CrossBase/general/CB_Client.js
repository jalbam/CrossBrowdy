...
var CB_Client = function() { return CB_Client; };
{
	...
	CB_Client._language = null; //Keeps most preferred language.
	CB_Client._languages = null; //Keeps most preferred languages by order of preference.
	...
	CB_Client.init = function()
	{
		...
		CB_Client._updateLanguageAndLanguagesCache(); //Updates the cache for language and languages.
		
		//Sets the event that will change the language and languages cache when the language is changed (for some browsers):
		CB_Events.add(window, "languagechange", function() { CB_Client._updateLanguageAndLanguagesCache(); }, true, true, false);

		return CB_Client;
	}


	//Updates the cache for language and languages:
	CB_Client._updateLanguageAndLanguagesCache =
		function()
		{
			CB_Client._language = CB_Client._getLanguagePreferred(); //Calculates the most preferred language.
			CB_Client._languages = CB_Client._getLanguagesPreferred(); //Calculates the most preferred languages.
		};
	 ...
	 *  @param {boolean} [recalculate=false] - If set to false, it will use a cache instead of calculating all again.
	 ...
	 */
	CB_Client.getLanguage = function(allowNavigatorLanguages, recalculate, PHPAcceptedLanguagesFirst)
	{
		if (recalculate) { CB_Client._language = CB_Client._getLanguagePreferred(allowNavigatorLanguages, PHPAcceptedLanguagesFirst); } //Keeps most preferred language.
		return CB_Client._language;
	}
	 ...
	 */
	CB_Client.getLanguages = function(allowNavigatorLanguages, recalculate, PHPAcceptedLanguagesFirst)
	{
		if (recalculate) { CB_Client._languages = CB_Client._getLanguagesPreferred(allowNavigatorLanguages, PHPAcceptedLanguagesFirst); } //Keeps most preferred language.
		return CB_Client._languages;
	}
	...
	{
		if (typeof(process) === "object" && CB_isString(process.version))
		{
	...
	CB_Client.isRunningOnElectron = function()
	{
		...
		try
		{
			return CB_Client._isRunningOnElectronReturnCache = (typeof(navigator) === "object" && typeof(navigator.userAgent) === "string" && navigator.userAgent.indexOf("Electron") !== -1);
		}
