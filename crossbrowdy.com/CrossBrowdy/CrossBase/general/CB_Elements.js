...
var CB_Elements = function() { return CB_Elements; };
{
	 ...
	 */
	CB_Elements.id = CB_Elements.get = function(id, useCache)
	{
		...
		if (!useCache || typeof(CB_Elements._idCache[id]) === "undefined" || CB_Elements._idCache[id] === null)
		{
			...
			{
				CB_Elements._idCache[id] = document.querySelector("#" + id) || null;
			}
			else if (document.all)
			{
				if (typeof(document.all) !== "function")
				{
					CB_Elements._idCache[id] = document.all[id] || null;
				}
				else { CB_Elements._idCache[id] = document.all(id); }
			}
			else if (document.layers)
			{
				CB_Elements._idCache[id] = document.layers[id] || null;
			}
	 ...
	 */
	CB_Elements.insertContent = function(container, content, displayValue, checkValues, computed, onContentWritten, onShow)
	{
		if (container !== null)
		{
			if (!checkValues || container.innerHTML !== content)
			{
	 ...
	 */
	CB_Elements.insertContentById = function(containerId, content, displayValue, checkValues, computed, onContentWritten, onShow)
	{
		return CB_Elements.insertContent(CB_Elements.id(containerId), content, displayValue, checkValues, computed, onContentWritten, onShow);
	}
	...
	CB_Elements.toggleClass = function(element, classA, classB, onToggleClass)
	{
		if (typeof(element) !== "undefined" && element !== null)
		{
			...
			{
				if (classB !== "" && classesUsed.indexOf(" " + classB + " ") !== -1) { classesUsed = classesUsed.replace(regularExpressionString(" " + classB + " ", true, true), " " + classA + " "); }
			...
			{
				classesUsed = classesUsed.replace(regularExpressionString(" " + classA + " ", true, true), " ");
	...
	CB_Elements.removeClass = function(element, className, checkValues, onRemoveClass)
	{
		if (typeof(element) !== "undefined" && element !== null)
		{
			...
			{
				element.className = CB_trim(classesUsed.replace(regularExpressionString(" " + className + " ", true, true), " "));
	...
	/**
	 * Prevents the possibility of selecting the content of a given element (makes it unselectable).
	 *  @function
	 *  @param {Node} element - The element which we do not want to be selectable.
	 *  @returns {Node|null} Returns the given element again or null.
	 */
	CB_Elements.preventSelection = function(element)
	{
		if (typeof(element) === "undefined" || element === null) { return null; }
	...
	/**
	 * Prevents the possibility of selecting the content of a given element (makes it unselectable), by its ID.
	 *  @function
	 *  @param {string} elementId - The identifier of the element which we do not want to be selectable.
	 *  @returns {Node|null} Returns the affected element (if any) or null otherwise.
	 */
	CB_Elements.preventSelectionById = function(elementId)
	{
		return CB_Elements.preventSelection(CB_Elements.id(elementId));
	}


	/**
	 * Disables the contextual menu for a given element or in the whole document.
	 *  @function
	 *  @param {Node} [element=document] - The element whose contextual menu we want to disable. If not given, it will affect the whole document.
	 *  @returns {Node} Returns the affected element.
	 */
	CB_Elements.contextMenuDisable = function(element)
	{
	...
	/**
	 * Disables the contextual menu for a given element (by its identifier).
	 *  @function
	 *  @param {string} elementId - The identifier of the element whose contextual menu we want to disable.
	 *  @returns {Node|null} Returns the affected element (if any) or null otherwise.
	 */
	CB_Elements.contextMenuDisableById = function(elementId)
	{
		var element = CB_Elements.id(elementId);
		if (element !== null)
		{
			return CB_Elements.contextMenuDisable(element);
		}
		return null;
	}
	 ...
	 *  @param {function} [returnNullOnFail=false] - If it is set to true, it will return null instead of zero (0) in the case that something goes wrong.
	 *  @returns {number|null} It could return zero (0) if something fails (use "returnNullOnFail" set to true to return null when it fails).
	 */
